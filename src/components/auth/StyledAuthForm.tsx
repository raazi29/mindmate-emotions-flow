...
const StyledAuthForm: React.FC<Props> = ({ initialMode = 'signin', redirectTo = '/dashboard' }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signin') {
        ...
        if (!error) {
          // On sign up Supabase may require email confirmation; still navigate to dashboard for now
    ...
    <StyledWrapper>
      <form className="form" onSubmit={handleSubmit}>
        <div className="flex-column">
          <label>{mode === 'signin' ? 'Email' : 'Email'}</label>
        </div>
        ...
        <div className="flex-row">
          ...
          <span className="span" onClick={()=>alert('Password reset is available from profile menu.')}>Forgot password?</span>
        </div>

        <button type="submit" className="button-submit" disabled={submitting || loading}>
          {mode === 'signin' ? 'Sign In' : 'Sign Up'}
        </button>

        <p className="p">
          {mode === 'signin' ? (
            <>Don't have an account? <span className="span" onClick={()=>setMode('signup')}>Sign Up</span></>
          ) : (
            <>Already have an account? <span className="span" onClick={()=>setMode('signin')}>Sign In</span></>
          )}
        </p>
        ...
        <div className="flex-row">
          <button className="btn google" type="button" onClick={onGoogle}>
            <svg version="1.1" width={20} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="#FBBB00" d="M113.47,309.408L95.648,375.94l-65.139,1.378C11.042,341.211,0,299.9,0,256
